...
The Terraform provider hardcodes `e` and carries a TODO admitting it. That is correct only for east-something regions and silently wrong elsewhere — and the capture contains real traffic to exactly the hostname it would fail to construct. A wrong prefix does not produce a helpful error; it produces `NXDOMAIN`.
