...
A note on a figure that has been quoted the other way: the assembly holds 237 distinct URL templates, which looks like far more than 148. It is not. Every Kiota request builder declares its template twice, once per constructor, and 89 of the 237 belong to navigation-only builders — fluent-API intermediates such as `{+baseurl}/analytics` that carry no HTTP method. Counting raw template literals gives 475; counting the ones an HTTP verb is actually issued against gives 148.
